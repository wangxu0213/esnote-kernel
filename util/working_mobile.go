...
func BootMobile(container, appDir, workspaceBaseDir, lang string) {
	...
	userHomeConfDir := filepath.Join(HomeDir, ".config", "esnote")
	...
	defaultWorkspaceDir := filepath.Join(workspaceBaseDir, "esnote")
	...
	if gulu.File.IsDir(workspaceBaseDir) {
		...
		if nil != err {
			...
			if oldConf && oldData && oldTemp {
				for _, entry := range entries {
					if "home" == entry.Name() || "esnote" == entry.Name() {
						continue
					}

					from := filepath.Join(workspaceBaseDir, entry.Name())
					to := filepath.Join(workspaceBaseDir, "esnote", entry.Name())
